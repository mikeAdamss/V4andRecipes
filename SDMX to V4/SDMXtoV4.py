...
# The different 'modes; you can run the tool in. This is passed on the command line by sys argument 1.
modes = {
...
# We're asking the user to give us dimension choices (and ins ome cases identify names of standard time imensions) on the command line
# we need to parse that string into something we can use
def parseChoices(choices):

    time = None   # unless there is
    geo = None
    obs = None
    dimensions = []
    ...
    if "time=" in choices:
        time = choices.split("time=").split(' ')[0].strip()
        print(time)
...
# TODO - unsure if we need to alter the input time structure
def ONStime(cell):
    return cell
...
def BUILDV4(file, opString):
    ...
    V4['Time_codelist'] = df[time].apply(ONStime)
    V4['Time'] = df[time].apply(ONStype)
