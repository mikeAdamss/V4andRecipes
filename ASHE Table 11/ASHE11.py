...
"""
Databaker script to create dataset: ASHE Table 10
...
def makeV4(cvDf, df):
    ...
    # Check that duplicates are for the expected nations only.  
    obs_file['duplicates'] = obs_file.duplicated()
    dupes = obs_file[obs_file['duplicates'] == True]
    dupes = dupes['Geography_codelist'].unique()
    for dup in dupes:
        if dup not in ['E92000001','W92000004','S92000003']:
            raise ValueError ("Unexpected duplicate row (with geo {u}) detected while removing expected geography repeats '['E92000001' 'W92000004' 'S92000003']'. Please correct and try again.".format(u=dup))
            
    # so its just the nations to remove, so remove them
    obs_file = obs_file[obs_file['duplicates'] == False]
    obs_file = obs_file.drop('duplicates', axis=1)
...
for dset in [hoursFiles, earningsFiles]:
    ...
    for ncf in dset:
        ...
        for tab in tabs:
            ...
            if 'notes' not in tab.lower():
                ...
                assert len(footer) == 1, "Not Classified'. Unable to find end of data table"
    ...
    df.to_csv('ASHE_11_{ds}{p}{t}.csv'.format(ds=ds, t=time, p=prov), encoding="utf-8", index=False)
